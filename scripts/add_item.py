...
def _canonicalize_link(link: str) -> str:
    """Return the URL without common tracking query parameters."""
    if not link:
        return link
    try:
...
def insert_item(channel, title, link, desc, guid=None, pubdate=None, max_items=50):
    ...
    # Canonicalize link (remove UTMs etc.) so duplicates don't sneak in
    ...
    # De-dup: skip if GUID already exists (optional: update instead)
    existing_items = channel.findall("item")
    existing_by_guid = {i.findtext("guid"): i for i in existing_items}
    if stable_guid in existing_by_guid:
        # OPTIONAL: update-in-place instead of skipping:
